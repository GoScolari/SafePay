...
- [x] `app/(auth)/login.tsx` — input teléfono, botón "Enviar código" → `POST /auth/login`
- [x] `app/(auth)/register.tsx` — input nombre + teléfono → `POST /auth/register`
- [x] `app/(auth)/verify-otp.tsx` — input 6 dígitos OTP, reenviar, submit → `POST /auth/verify-otp` → guarda tokens
...
- [x] `app/(app)/index.tsx` — lista con React Query `GET /transactions/my`, pull-to-refresh, FAB "Nueva", empty state
...
- [x] `app/(app)/transactions/[id].tsx` — detalle completo con acciones condicionales por estado/rol
- [x] Acción PROPUESTA: "Aceptar" (contraparte), "Copiar link" + "Cancelar" (iniciador)
- [x] Acción CONFIRMADA: "Ir a pagar"
- [x] Acción PAGADO: "Registrar envío" (vendedor), "Cancelar"
...
- [x] `components/StatusStepper.tsx` — barra visual de progreso + estados especiales
- [x] `components/ActionButton.tsx` — botón con variantes primary/danger/outline
- [x] `hooks/useTransaction.ts` — React Query + mutaciones accept/cancel/release + copyLink
- [x] `npx tsc --noEmit` sin errores
- [ ] Flujo probado contra backend local
...
- [x] `app/(app)/transactions/tracking.tsx` — selector courier + input tracking number
...
- [ ] Flujo completo E2E probado: register → login → crear tx → aceptar → pagar → tracking → confirmar → completado
