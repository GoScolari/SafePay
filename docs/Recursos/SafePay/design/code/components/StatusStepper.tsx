/**
 * SafePay · StatusStepper
 *
 * Timeline vertical de los estados de la transacción. Dos flows happy-path:
 *
 *   shipping    · PROPUESTA → CONFIRMADA → PAGADO → EN_TRANSITO → ENTREGADO → COMPLETADO
 *   presential  · PROPUESTA → CONFIRMADA → PAGADO → ENTREGADO → COMPLETADO  (skip EN_TRANSITO)
 *
 * Estados especiales reemplazan visualmente el siguiente step:
 *
 *   EXPIRADO    · desde PROPUESTA (sin respuesta)
 *   CANCELADO   · desde PAGADO (antes del despacho)
 *   EN_DISPUTA  · desde ENTREGADO (comprador reportó)
 *   REEMBOLSADO · desde EN_DISPUTA (resuelto a favor del comprador)
 *
 * Los pasos previos al punto de divergencia se marcan como `done`. El step
 * alternativo lleva su propio color (gris para cancel/expire, naranja
 * para disputa, rojo para reembolso) y termina el timeline.
 */
...
interface StatusStepperProps {
  ...
  status: TxStatus;
  /** Flow base (cambia si la tx incluye EN_TRANSITO o no) */
  flow: StepperFlow;
  /** Mapa opcional de timestamps por estado, formateados para mostrar */
  timestamps?: Partial<Record<TxStatus, string>>;
  style?: StyleProp<ViewStyle>;
}

// ─── Definición de los flows happy-path ────────────────────────────────────
...
/** Estados que terminan el timeline con un final alternativo. */
const ALTERNATE_ENDINGS = new Set<TxStatus>(['EXPIRADO', 'CANCELADO', 'EN_DISPUTA', 'REEMBOLSADO']);

/** Hasta qué punto del happy-path llegó la tx antes de divergir. */
...
function computeSteps(
  ...
  // Si el estado actual es happy-path, mostramos el flow completo con el
  // current marcado y los siguientes en `future`.
  ...
  // Estado alternativo: cortamos el flow en el punto de divergencia y
  // anexamos el step alternativo.
...
function defaultFutureMeta(stepStatus: TxStatus, currentStatus: TxStatus): string | undefined {
  // Hints contextuales para steps todavía por ocurrir
...
function getStepTone(step: ComputedStep): StepTone {
  ...
  switch (step.status) {
    case 'EN_DISPUTA':
      return {
        dot: { backgroundColor: Colors.warning, borderColor: Colors.warning },
